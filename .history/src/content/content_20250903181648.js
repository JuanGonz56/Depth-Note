...
class DepthAnnotationTool {
  ...
  setupMessageListener() {
    ...
    chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
      ...
      } else if (message.action === 'highlightAnnotation') {
        this.highlightAnnotation(message.annotationId);
        sendResponse({ success: true });
      } else if (message.action === 'toggle3DView') {
        this.toggle3DView();
        sendResponse({ success: true });
      } else if (message.action === 'quickAnnotate') {
        this.createAnnotation(message.x, message.y);
        sendResponse({ success: true });
      }
  ...
  createFloatingToolbar() {
    ...
    this.toolbar.innerHTML = `
      <div class="toolbar-content">
        <button id="toggle-annotations" class="toolbar-btn">
          📝 Annotate
        </button>
  ...
  createAnnotation(x, y) {
    ...
    // Auto-focus the textarea and show popup
  ...
  saveAnnotation(annotationId, text) {
    ...
    if (annotationIndex !== -1) {
      ...
      this.saveToStorage();
      console.log('Annotation saved:', annotationId, text);
    }
  }
  ...
  highlightAnnotation(annotationId) {
    ...
    if (annotation) {
      ...
      // Highlight effect
      annotation.style.animation = 'highlight 2s ease-in-out';
      setTimeout(() => {
  ...
  loadAnnotations() {
    ...
    chrome.storage.local.get([key], (result) => {
      if (result[key] && result[key].annotations) {
        console.log('Loading existing annotations:', result[key].annotations.length);
...
style.textContent = `
  @keyframes highlight {
    0% { transform: scale(1); }
    50% { transform: scale(1.2); box-shadow: 0 0 30px var(--depth-color); }
